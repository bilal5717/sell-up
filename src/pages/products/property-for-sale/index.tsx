...
const PropertyForSalePage: React.FC = () => {
  ...
  useEffect(() => {
    // Parse the URL to set the selected sub-category on page load
    const pathParts = pathname.split('/');
    const subCategorySlug = pathParts[pathParts.length - 1];
    const validSlugs = subCategories['property-for-sale'].map(sub => sub.slug);
    if (validSlugs.includes(subCategorySlug) && subCategorySlug !== 'property-for-sale') {
      const subCategory = subCategories['property-for-sale'].find(sub => sub.slug === subCategorySlug);
      if (subCategory) {
